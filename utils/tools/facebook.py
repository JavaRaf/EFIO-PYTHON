...
class Facebook:
    ...
    @retry(
        ...
        wait=wait_exponential(multiplier=1, min=2, max=10),  # Tempo de espera exponencial
        retry=retry_if_exception_type(httpx.HTTPError),  # Só tenta novamente se for erro HTTP
    ...
    def post(self, message: str = "", frame_path: Path = None, parent_id: str = None) -> str | None:
        ...
        """
        endpoint = f"{self.base_url}/{parent_id}/comments" if parent_id else f"{self.base_url}/me/photos"
    ...
    def repost_to_album(self, message: str, frame_path: Path, configs: dict, counter: dict) -> str | None:
        """
    ...
    def update_bio(self, message: str) -> str | None:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Failed to update bio: {response.status_code} {response.text}")
